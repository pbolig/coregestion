...
router.put('/:id/estado', authenticateToken, authorizeRoles(['admin', 'ventas']), async (req, res) => {
    ...
    try {
        await db.run('BEGIN TRANSACTION');
        const presupuesto = await db.get('SELECT cliente_id, total, estado FROM presupuestos WHERE id = ?', [id]);
        ...
        if (!validTransitions[estadoActual] || !validTransitions[estadoActual].includes(nuevo_estado)) {
            await db.run('ROLLBACK');
            return res.status(409).json({ message: `Acción no permitida: No se puede cambiar de "${estadoActual}" a "${nuevo_estado}".` });
        }

...
router.post('/register', async (req, res) => {
    ...
    try {
        const existingProspecto = db.prepare('SELECT id FROM prospectos WHERE email = ?').get(email);
        ...
        const stmt = db.prepare(`INSERT INTO prospectos (nombre, empresa, email, telefono, password, estado) VALUES (?, ?, ?, ?, ?, 'Pendiente')`);
        stmt.run(nombre, empresa, email, telefono, hashedPassword);
...
router.post('/login', async (req, res) => {
    ...
    try {
        const prospecto = db.prepare('SELECT * FROM prospectos WHERE email = ?').get(email);
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        if (prospecto) {
            ...
            db.prepare("UPDATE prospectos SET reset_token = ?, reset_token_expires = ? WHERE id = ?").run(resetToken, tokenExpires, prospecto.id);
...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        const prospecto = db.prepare("SELECT * FROM prospectos WHERE reset_token = ? AND reset_token_expires > ?").get(token, Date.now());
        if (!prospecto) return res.status(400).json({ message: "El token es inválido o ha expirado." });

        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);
        db.prepare("UPDATE prospectos SET password = ?, reset_token = NULL, reset_token_expires = NULL WHERE id = ?").run(hashedPassword, prospecto.id);
...
router.post('/google-auth', async (req, res) => {
    ...
    try {
        ...
        let prospecto = db.prepare('SELECT * FROM prospectos WHERE email = ?').get(email);
        if (!prospecto) {
            db.prepare(`INSERT INTO prospectos (nombre, email, password, estado) VALUES (?, ?, ?, ?)`).run(name, email, 'google-authenticated', 'Pendiente');
            prospecto = db.prepare('SELECT * FROM prospectos WHERE email = ?').get(email);
        }

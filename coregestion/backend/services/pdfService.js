...
function generateHeader(doc) {
    doc.fillColor('#444444')
        .fontSize(20)
        .text('CoreGestión', 50, 57)
        .fontSize(10)
        .text('Dirección de tu Empresa', 200, 65, { align: 'right' })
        .text('Ciudad, Provincia, CP', 200, 80, { align: 'right' })
        .moveDown();
}

function generateCustomerInformation(doc, cliente, factura) {
    // --- LÓGICA MEJORADA ---
    // Determina el título y número de comprobante a mostrar
    let tipoComprobante = 'Remito / Ppto. Facturado';
    let numeroComprobante = `${String(factura.punto_venta).padStart(5, '0')}-${String(factura.numero_comprobante).padStart(8, '0')}`;
    ...
    doc.fontSize(10)
        .text('Comprobante N°:', 50, customerInfoTop)
        .font('Helvetica-Bold')
        .text(numeroComprobante, 150, customerInfoTop)
        .font('Helvetica')
        .text('Fecha de Emisión:', 50, customerInfoTop + 15)
        .text(fechaComprobante.toLocaleDateString('es-AR'), 150, customerInfoTop + 15)
        .text('Saldo Pendiente:', 50, customerInfoTop + 30)
        .text(`$${factura.saldo_pendiente.toFixed(2)}`, 150, customerInfoTop + 30)

        .font('Helvetica-Bold')
        .text(cliente.nombre, 300, customerInfoTop)
        .font('Helvetica')
        .text(cliente.direccion || '', 300, customerInfoTop + 15)
        .text(`${cliente.email || ''}`, 300, customerInfoTop + 30)
        .moveDown();
...
function generateInvoiceTable(doc, presupuesto, gastos) {
    ...
    presupuesto.insumos.forEach(item => {
        ...
        generateTableRow(doc, position, item.nombre, `$${item.precio_unitario.toFixed(2)}`, item.cantidad, `$${itemTotal.toFixed(2)}`);
        position += 20;
    });
    ...
    if (gastos && gastos.length > 0) {
        ...
        gastos.forEach(gasto => {
            generateTableRow(doc, position, gasto.concepto, `$${gasto.monto.toFixed(2)}`, '1', `$${gasto.monto.toFixed(2)}`);
            position += 20;
        });
    }

    generateHr(doc, position - 10);
    const subtotal = presupuesto.total;
    ...
    if (gastos && gastos.length > 0) {
        doc.font('Helvetica-Bold').fontSize(10)
            .text('Gastos Adicionales:', 400, position + 20, { align: 'left' })
            .text(`$${totalGastos.toFixed(2)}`, 0, position + 20, { align: 'right' });
        position += 15;
    }
